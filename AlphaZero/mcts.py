...
class MCTSNode:
    ...
    def __init__(self, state, n_actions, TreeEnv, action=None, parent=None):
        ...
        self.child_W = np.zeros([n_actions], dtype=np.float32) # Save copy of original prior before it gets mutated by dirichlet noise
    ...
    def select_leaf(self):
        ...
        while True:
            ...
            index, best_move = current.get_best_action()
            current = current.maybe_add_child(index, best_move)
        return current

    def get_best_action(self):
        p = self.child_action_score
        ...
        if action is None:
            ...
            index = np.argmax(self.TreeEnv.ae.transform([action.name]).sum(axis=0))
            #if p[index]<0:
             #   action=None
        return index, action
    ...
    def inject_noise(self, diri):
        dirch = np.random.dirichlet([diri] * self.n_actions)
...
class MCTS:
    ...
    def tree_search(self, num_parallel=None):
        ...
        while len(leaves) < num_parallel and failsafe < num_parallel:
            ...
            if leaf.is_done():
                value, _ = self.TreeEnv.get_return(leaf.state.head, leaf.depth)
    ...
    def take_action(self, index, action):
        ...
        #reward = self.root.children[index].depth
        reward, _ = (self.TreeEnv.get_return(self.root.children[index].state.head,
                                          self.root.children[index].depth))
        #self.rewards.append(reward)
        self.obs.append(HelperTrainingExample(deepcopy(self.root.state), self.TreeEnv.goal,
                                               action_proba, reward, reward))
...
def execute_episode(agent_netw, num_simulations, TreeEnv):
    ...
    probs, vals = agent_netw.step(TreeEnv.get_obs_for_state(first_node.state))
    ...
    while True:
        ...
        dirch = min(10/len(possible_action), 1)
        mcts.root.inject_noise(dirch)
        ...
        while mcts.root.N < current_simulations + num_simulations:
            mcts.tree_search(num_parallel = num_parallel)
    ...
    ret, _ = TreeEnv.get_return(mcts.root.state.head, mcts.root.depth)
    print(ret)
    ...
    return (mcts.obs, mcts.searches_pi, ret, total_rew, mcts.root)
