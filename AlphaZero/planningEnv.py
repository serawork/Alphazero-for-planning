...
class PlanningAgent(StaticEnv):
    ...
    """
    def __init__(self, n_actions, task, ae, dict_fact,
    ...
    def step(self, action):
        ...
        done = self.task.goal_reached(ns) or self.step_idx>=self.ep_length
    ...
    def get_return(self, state, step_idx):
        ...
        :param step_idx: number of action taken from the initial state
        :return: the reward value
        """  
        # Reward is one if goal reached, zero otherwise
        if self.task.goal_reached(state):
            reward = 1
        else:
            reward=0

        # Uncomment to shape the reward as inverse of heuristic value.
        """
    ...
    def get_obs_for_state(self, state):
        ...
        #E = np.reshape(E, (1, E.shape[0], E.shape[1], E.shape[-1]))
        return X
    ...
    def next_state(state, action):
        ...
        #n_state= n_state.union(state.head)
